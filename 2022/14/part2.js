...
const nextSandPos = (map, pos, floorY) => {
  return DIR_DELTAS.map((d) => pos.add(d)).find((p) => {
    return p.im != floorY && map.get(p.key()) == SYM.AIR;
  });
};
...
while (sandFlows) {
  ...
  let nextPos = nextSandPos(map, pos, floorY);
  while (nextPos && map.get(POS_SOURCE.key()) == SYM.SOURCE) {
    pos = nextPos;
    nextPos = nextSandPos(map, pos, floorY);
  }
  if (map.get(POS_SOURCE.key()) != SYM.SOURCE) {
    sandFlows = false;
  } else {
