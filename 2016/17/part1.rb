...
def shortest_path(passcode, path=[], row=0, col=0)
  ...
  DIRECTIONS.each_with_index do |d, i|
    if door_open(row+d[0], col+d[1], hash[i])
      paths << shortest_path(passcode, path.dup.append(INDEX2DIR[i]), row+d[0], col+d[1])
    end
  end

  paths.reject(&:nil?).min_by(&:length)
end

puts shortest_path(passcode).join
