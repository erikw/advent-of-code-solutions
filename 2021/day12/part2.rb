...
def find_paths(nodes, node_cur, paths_all, path_cur)
  ...
  if node_cur == nodes['end']
    ...
    node_cur.neighbours.each do |neighbour|
      has_small_twice = path_cur.select(&:is_small?).group_by(&:itself).map(&:last).map(&:length).max == 2
      if neighbour.name != 'start' && (!neighbour.is_small? || !has_small_twice || !path_cur.include?(neighbour))
