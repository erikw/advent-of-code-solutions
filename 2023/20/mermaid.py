...
def read_module_configuration():
    ...
    for name in mod_conf:
        for dest_name in mod_conf[name]["dest_names"]:
            if (
                dest_name in mod_conf
                and mod_conf[dest_name]["type"] == TYPE_CONJUNCTION
            ):
...
def main():
    ...
    nodes = {MODULE_FINAL: Node(MODULE_FINAL, f"{TYPE_UNTYPED}{MODULE_FINAL}")}
    links = []
    ...
    chart = MermaidDiagram(
        title="Module Configuration", nodes=list(nodes.values()), links=links
    )
